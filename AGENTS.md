...
- **后台启动**：`cmd.exe` 作为锚点进程；`subprocess.Popen(command, shell=True, cwd=cwd, env=env, stdout=log_fd, stderr=STDOUT, stdin=DEVNULL, creationflags=CREATE_NO_WINDOW | CREATE_NEW_PROCESS_GROUP)`。`shell=True` 是必要的：list 形式经 `subprocess.list2cmdline` 转义后含 4 个引号，cmd `/s` 会剥离首尾引号破坏带内层引号的命令；`shell=True` 让 Popen 把 command 直接交给 cmd.exe，cmd 仍作为锚点（Job Object / `process_tree` 基于此 pid）。`runToken` 通过 `env["CONSOLE_RUN_TOKEN"]` 注入；token 还在 cmd 锚点 argv 的注释行 `rem console-run:<token>` 中保留，便于调试与溯源。
